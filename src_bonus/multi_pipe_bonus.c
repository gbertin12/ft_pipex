...
/*   Updated: 2022/08/09 20:34:29 by gbertin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_exec_cmd(t_list_bonus *cmd, t_struct *pipex, char *envp[])
{
	...
	if (pipe(pip) == -1)
 		return (0);
	if (cmd->next)
		exec_first_mid_cmd(pip, cmd, pipex, envp);
	else
...
int exec_first_mid_cmd(int pip[2], t_list_bonus *cmd, t_struct *pipex, char *envp[])
{
	int pid;
